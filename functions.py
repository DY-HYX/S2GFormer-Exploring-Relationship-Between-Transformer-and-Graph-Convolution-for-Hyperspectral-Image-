...
def gain_neighborhood_pixel(pca_image, point, i, patch, W, H):
    ...
    cornor_index=torch.Tensor(cornor_index)
...
def train_and_test_data(pca_image, band, train_point, test_point, true_point, patch, w, h):
    ...
    corner_train = torch.zeros((train_point.shape[0], 2), dtype=torch.float32).to(device)
    corner_test = torch.zeros((test_point.shape[0], 2), dtype=torch.float32).to(device)
    corner_true = torch.zeros((true_point.shape[0], 2), dtype=torch.float32).to(device)
...
#-------------------------------------------------------------------------------
# train model
def train_epoch(tr_net, train_loader, criterion, optimizer):
    objs = AvgrageMeter()
    top1 = AvgrageMeter()
    tar = np.array([])
    pre = np.array([])
    for batch_idx, (batch_data, batch_target, center_pos,A,D) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        batch_pred = tr_net(batch_data,center_pos,D)
...
#     D2 =torch.from_numpy(B).sum(1).round().type(torch.int).cuda()
#     return A, D, D_max,D2
...
#         if x==1 and y==1:
#             edge_number[4]=edge_number[4]+1
#         if x==1 and y==-1:
#             edge_number[5]=edge_number[5]+1
#         if x==-1 and y==1:
#             edge_number[6]=edge_number[6]+1
#         if x==-1 and y==-1:
#             edge_number[7]=edge_number[7]+1
#
#     return edge_number
#
#
# def get_edge_A(patch):
#     edge_A=np.zeros((patch*patch,patch*patch,8))
...
#     edge_A=torch.reshape(edge_A,(patch*patch*patch*patch,8))
